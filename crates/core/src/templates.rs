...
pub fn render_env_example(t: &Template) -> String {
    ...
    out.push_str(&format!(
        "# .env.example for the '{}' template — variable names only, NO values.\n\
         # Copy to .env for local use, or prefer `tethra run` so secrets\n\
         # never land on disk. Generated by Tethra; contains no secrets.\n\n",
        t.id
    ));
    for v in &t.env_vars {
        ...
        if v.secret {
            out.push_str("# SECRET — store in the Tethra vault, do not commit a value.\n");
        }
        out.push_str(&format!("{}=\n\n", v.name));
    }
    out
...
pub fn next_steps(t: &Template, project: &str) -> Vec<String> {
    ...
    for v in t.env_vars.iter().filter(|v| v.secret) {
        ...
        steps.push(format!(
            "tethra key add --project {project} --name {key_name} --provider {provider}"
        ));
        steps.push(format!(
            "tethra mapping set --project {project} {project}/{key_name} {}",
            v.name
        ));
    }
    steps
...
mod tests {
    ...
    fn env_example_contains_names_and_no_values() {
        ...
        assert!(example.contains("SECRET — store in the Tethra vault"));
