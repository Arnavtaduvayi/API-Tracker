...
| `RA-003` | HIGH | **FIXED, and now OBSERVED PASSING** | Count gates derived from the registered checks; enumerator self-test wired into `--scope selfcheck`. The structural fix made the scope report `59/59` instead of INCONCLUSIVE; a clean *pass* needed a machine with no installed Tethra gateway, which no machine here is. It now runs on a disposable hosted macOS runner on every PR and passes **63/63** (run `30325704492`). The total moved 59 → 63 because the first real execution exposed `REM-003`/`REM-004`, and it moved without anyone editing a total — see `SERVICE_VALIDATION_EVIDENCE.md` §7 |
...
|---|---|---|---|
| `REM-002` | HIGH | **FIXED** | `tracking_validate_macos.sh` armed `trap cleanup EXIT` **before** its mode preconditions ran, so a refusal fired a destructive teardown. Latent rather than exploited — teardown was already bounded by `SERVICE_INSTALLED`, so a refusal found nothing to remove — but it is the `RA-004` shape surviving in the second script after the first had been fixed. One script having the safe design is not the same as the design being the house style. The preflight now runs every refusal above the trap, the ledger is general (dir / plist / label / pid) and re-proves ownership before acting, and a production-definition signature is compared across every exit path including refusals. |
| `REM-003` | MEDIUM | **FIXED** | `the installed LaunchAgent runs the bundled helper` asserted something the product does not do: `gateway install` copies the running CLI to `<data-dir>/bin/tethra-gateway-<version>` and points the plist there, so the bundle path never appears in it. The check had **never executed anywhere**, because `--require-service` had never completed. Replaced by two stronger assertions (the plist runs the copy installed in this run's data directory; that copy is byte-identical to the in-bundle helper). Found by the first clean-runner execution. |
| `REM-004` | LOW | **FIXED** | the privacy header sweep matched `authorization:` / `Bearer ` inside the installed helper **binary**, which necessarily carries them as string constants. Service mode puts that binary inside `$TETHRA_DIR`; foreground mode never does. A false positive, not a leak: the two per-run needles found nothing. Excluded from the header sweep only, and sound because of `REM-003`'s fix — the binary is proven byte-identical to the shipped one. |
| `REM-005` | MEDIUM | **FIXED** | `gateway_validate_macos.sh` gated its attribution-distinctness assertions on a `matching_key_present` field that `--json gateway status` **has never emitted** (it lives on the control-channel status; the doctor document is a different shape, and pretty-printed). The gate was false on every run for the life of the check, so the assertions never ran and the else-branch reported a permanent coverage gap blamed on a missing TTY — while push-key had succeeded every time. A check that can never run is the `ZFT-VAL-7` shape inverted, and the count floor cannot see it. Gate removed; assertions unconditional. |
| `REM-006` | MEDIUM | **FIXED** | the `RA-004` evidence quoted sha256 pairs, a `launchctl` invocation log and a four-row mutation table from a harness that **was never committed**. A claim whose control cannot be re-run is a claim, not evidence — the same defect class as an uncounted check. `scripts/service_cleanup_safety.sh` is that harness, committed, 11 assertions, and it still reports the honest negative result about the two layers. |
| `REM-001` | HIGH | **FIXED** | `tracking_validate_macos.sh --scope full --require-service` stopped this machine's live production gateway. Its interlock globs `$HOME/Library/LaunchAgents`; `launchctl` addresses `gui/<uid>`, which `$HOME` does not isolate — so redirecting `HOME` (the previous auditor's own documented technique) walks past the interlock while every `launchctl` call still lands on the user's real service. Same shape as `RA-004`, in a second script, invisible until `RA-003` was fixed and service mode could run at all. The interlock now asks launchd as well as the filesystem, verified against the exact condition that caused the damage. The user's gateway was restored from its own unmodified plist and is running; it was down for about six minutes. Full record in `FINAL_REMEDIATION_EVIDENCE.md` §4b. **Follow-through:** the scope that caused this now runs only where it is safe — a disposable hosted runner with no Tethra installation — and the developer machine's gateway was verified unchanged (same pid, same plist bytes, same mtime) before and after every local step of this work. |
