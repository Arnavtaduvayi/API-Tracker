...
**Superseded in part (2026-07-29, SEC-01 / NEW-49).** D3's claim that the route table "is not the trust root for where a credential goes" holds only for the *destination* of a custom origin, not for the *selector* of a built-in one. `gateway_routes.provider_id` is stored, unauthenticated, and chooses which compiled-in manifest origin a built-in route resolves to, so an attacker with local write access to `vault.db` can reassign a built-in route among the shipped provider origins with the pass-through credential attached, or null all four custom columns to downgrade a MAC'd custom row onto that same unauthenticated path. This is an accepted, out-of-scope risk, not a defended one. The decision recorded below is unchanged and was not reversed; the two places where its wording overstated it are marked `[Amended …]` inline. The governing statements are now `docs/gateway/SECURITY.md` ("What database tampering can and cannot do to your routes"), `docs/gateway/THREAT_MODEL.md` GW-3, and `docs/activity-onboarding/SECURITY_AND_PRIVACY.md` ("the local-database attacker").
