...
nothing but sanitized metadata. The observation proxy has no cloud or
analytics component and has no capability to record request or response
...
Optional desktop product analytics is separate from request observation,
consent-gated, and receives no observability data (`docs/ANALYTICS.md`).
